...
Eigen::Vector3<T> RotateAxisAngle(const Eigen::Vector3<T>& vec,
                                  ...
                                  const T& angle) {
  internal::ThrowIfNotUnitVector(axis, __func__);
  const T cos_ang = cos(angle);
  const T sin_ang = sin(angle);
...
T SignedAngleAroundAxis(const Eigen::Vector3<T>& vec1,
  ...
  const T angle = atan2(vec1.cross(vec2).dot(axis), vec1.dot(vec2));
  return angle;
}
