...
class ShoelaceTyingStation(Diagram):
    def __init__(self, plant_dt=0.001, diff_ik_dt=0.002,
        ...
        # Add shoe
        shoelace_bodies = AddShoe(robot_builder, ground_geom_id, parallelism=parallelism, **kwargs)

        robot_diagram = robot_builder.Build()
        plant = robot_diagram.plant()

        # Set the default configuration of the arms
        q0 = [0.0, -np.pi/4, 0., -3*np.pi/4, 0.0,
              np.pi/2, np.pi/4, 0.04, 0.04] * 2
        ...
        collision_checker = SceneGraphCollisionChecker(CollisionCheckerParams(
        ...
        differential_inverse_kinematics = DifferentialInverseKinematicsSystem(
            ...
            K_VX=1.0,
            Vd_TG_limit=SpatialVelocity(w=rotational_velocity_limit, v=translational_velocity_limit),
        )
        ...
        # Connect differential inverse kinematics controller, joint stiffness controller, and plant
        ...
        builder.Connect(robot_diagram.GetOutputPort(f"plant_{plant.GetModelInstanceName(left_arm)}_state"),
                        demux1.get_input_port())
        builder.Connect(robot_diagram.GetOutputPort(f"plant_{plant.GetModelInstanceName(right_arm)}_state"),
                        demux2.get_input_port())
        builder.Connect(demux1.get_output_port(0), arm_state_mux.get_input_port(0))
        builder.Connect(demux1.get_output_port(1), arm_state_mux.get_input_port(2))
        builder.Connect(demux2.get_output_port(0), arm_state_mux.get_input_port(1))
        builder.Connect(demux2.get_output_port(1), arm_state_mux.get_input_port(3))

        builder.Connect(arm_state_mux.get_output_port(),
        ...
        builder.Connect(diff_ik_controller.GetOutputPort("commanded_position"),
                        demux1.get_input_port())
        builder.Connect(diff_ik_controller.GetOutputPort("commanded_velocity"),
                        demux2.get_input_port())
        ...
        builder.Connect(left_hand_finger_controller.get_output_port(),
                        mux.get_input_port(1))
        builder.Connect(demux1.get_output_port(1), mux.get_input_port(2))
        builder.Connect(right_hand_finger_controller.get_output_port(),
        ...
        builder.Connect(joint_stiffness_controller.get_output_port_actuation(
        ), robot_diagram.GetInputPort("plant_actuation"))

        # Connect nominal configuration input of differential inverse kinematics controller
        nominal_posture_source = builder.AddSystem(ConstantVectorSource(q0))
        builder.Connect(nominal_posture_source.get_output_port(),
                        diff_ik_controller.GetInputPort("nominal_posture"))

        # Connect desired pose input of differential inverse kinematics controller
        ...
        builder.Connect(bus_creator.get_output_port(),
        ...
        X_WH_left = plant.GetFrameByName(
            "panda_hand", left_arm).CalcPoseInWorld(plant_context)
        X_WH_right = plant.GetFrameByName(
            "panda_hand", right_arm).CalcPoseInWorld(plant_context)
        ...
        # Export output ports
        builder.ExportOutput(robot_diagram.GetOutputPort(
            "scene_graph_query"), "scene_graph_query")
    ...
    def GetHandGrasps(self, context):
        ...
        left_hand_grasp = 1.0 - (left_hand_finger1.GetOnePosition(plant_context) +
        ...
        right_hand_finger2 = plant.GetJointByName(
            "panda_finger_joint2", self._left_arm)
        right_hand_grasp = 1.0 - (right_hand_finger1.GetOnePosition(plant_context) +
    ...
    def GetFilamentNodePositions(self, context):
        ...
        plant = self._robot_diagram.plant()
        plant_context = self.GetSubsystemContext(plant, context)
        return np.hstack([filament_body.GetPositions(plant_context) for filament_body in self._shoelace_bodies])
...
def AddShoe(robot_builder, ground_geom_id,
            lace_length=0.5, lace_num_edges=99, lace_diameter=0.004,
            lace_youngs_modulus=2e4, lace_mass_density=50,
    ...
    X_WS = RigidTransform(RotationMatrix.MakeZRotation(-np.pi/2), [0.016, 0, 0.062])
    plant.WeldFrames(plant.world_frame(), shoe.body_frame(), X_WS)
    shape = Mesh("models/shoe.gltf", 0.06)
    plant.RegisterVisualGeometry(shoe, RigidTransform(), shape, "shoe", [0.5,0.5,0.5,1.0])
    shape = Convex("models/shoe_sole.obj", 0.06)
    plant.RegisterCollisionGeometry(shoe, RigidTransform(), shape, "shoe_sole", CoulombFriction(0.2, 0.2))
    shape = Convex("models/shoe_tongue.obj", 0.06)
    plant.RegisterCollisionGeometry(shoe, RigidTransform(), shape, "shoe_tongue", CoulombFriction(0.1, 0.1))

    if lace_as_capsule_chain:
        collision_filter_manager = robot_builder.scene_graph().collision_filter_manager()
        ...
        for i in range(lace_num_edges):
            name = f"link{i}"
            link = plant.AddRigidBody(name, left_lace_model_instance, SpatialInertia.SolidCapsuleWithDensity(
                lace_mass_density, lace_diameter/2, lace_edge_length, [0,0,1]))
            capsule = Capsule(lace_diameter/2, lace_edge_length)
            plant.RegisterVisualGeometry(link, RigidTransform(), capsule, name, lace_rgba.rgba)
            id = plant.RegisterCollisionGeometry(link, RigidTransform(), capsule, name, proximity_props)
            ...
            left_lace_head_frame.append(plant.AddFrame(
                FixedOffsetFrame(f"{name}_head_frame", link,
                                 RigidTransform([0, 0, lace_edge_length/2]))))
            left_lace_tail_frame.append(plant.AddFrame(
                FixedOffsetFrame(f"{name}_tail_frame", link,
                                 RigidTransform([0, 0, -lace_edge_length/2]))))

        for i in range(len(left_lace_geom_id)-1):
            collision_filter_manager.Apply(
                CollisionFilterDeclaration().ExcludeWithin(
                    GeometrySet([left_lace_geom_id[i], left_lace_geom_id[i+1]]))
            )
            plant.AddForceElement(LinearBushingRollPitchYaw(
                left_lace_tail_frame[i], left_lace_head_frame[i+1],
        ...
        for i in range(lace_num_edges):
            name = f"link{i}"
            link = plant.AddRigidBody(name, right_lace_model_instance, SpatialInertia.SolidCapsuleWithDensity(
                lace_mass_density, lace_diameter/2, lace_edge_length, [0,0,1]))
            capsule = Capsule(lace_diameter/2, lace_edge_length)
            plant.RegisterVisualGeometry(link, RigidTransform(), capsule, name, lace_rgba.rgba)
            id = plant.RegisterCollisionGeometry(link, RigidTransform(), capsule, name, proximity_props)
            ...
            if i == 0:
                ...
                plant.SetDefaultFreeBodyPose(link, tf)
            right_lace_head_frame.append(plant.AddFrame(
                FixedOffsetFrame(f"{name}_head_frame", link,
                                 RigidTransform([0, 0, lace_edge_length/2]))))
            right_lace_tail_frame.append(plant.AddFrame(
                FixedOffsetFrame(f"{name}_tail_frame", link,
                                 RigidTransform([0, 0, -lace_edge_length/2]))))
        ...
        for i in range(len(right_lace_geom_id)-1):
            collision_filter_manager.Apply(
                CollisionFilterDeclaration().ExcludeWithin(
                    GeometrySet([right_lace_geom_id[i], right_lace_geom_id[i+1]]))
            )
            plant.AddForceElement(LinearBushingRollPitchYaw(
                right_lace_tail_frame[i], right_lace_head_frame[i+1],
                torque_stiffness_constants=np.ones(3) * 1e-5,
                torque_damping_constants=np.ones(3) * 0,
                force_stiffness_constants=np.ones(3) * 10.0,
                force_damping_constants=np.ones(3) * 0,
    ...
    right_lace_prop = deformable_model.GetMutableBody(left_lace_body_id).mutable_der_model().mutable_structural_property()
    ...
    deformable_model.SetWallBoundaryCondition(
        ...
        p_WQ=X_WL @ lace.node_pos()[:,0] + np.array([0, lace_edge_length * 1.001, 0]),
        n_W=[0, 1, 0]
    )
    ...
    left_lace_prop = deformable_model.GetMutableBody(right_lace_body_id).mutable_der_model().mutable_structural_property()
    ...
    # Remove the collision between the rope and the floor and add a virtual field
    collision_filter_manager = robot_builder.scene_graph().collision_filter_manager()
    collision_filter_manager.Apply(
    ...
    deformable_model.AddExternalForce(FictiousFloor(
        floor_normal=[0, 0, 1],
...
def AddProximityProperties(proximity_props,
    ...
    proximity_props.AddProperty(
        "hydroelastic", "circumferential_resolution_hint", circumferential_resolution_hint)
    proximity_props.AddProperty(
        "hydroelastic", "longitudinal_resolution_hint", longitudinal_resolution_hint)
    AddContactMaterial(
        properties=proximity_props,
...
class FingerController(LeafSystem):
    ...
    def CalcOutput(self, context, output):
        ...
        grasp = input_port.Eval(context)[0] if input_port.HasValue(context) else 0.0
        grasp = max(0.0, min(1.0, grasp))
        output.SetFromVector((1.0 - grasp) * 0.04 * np.ones(2))


class FictiousFloor(ForceDensityField):
    def __init__(self, floor_normal, floor_point, mass_density,
                 stiffness=1e5, hunt_crossley_dissipation=0.01,
                 friction_coefficient=0.1, K=10.0):
        super().__init__()
    ...
    def DoEvaluateAt(self, context, p_WQ, v_WQ):
        return EvaluateFictiousFloorForceDensity(p_WQ, v_WQ,
                self._n_WF, self._p_WF, self._rho,
                self._k, self._d, self._mu, self._K)

    def DoClone(self):
        return FictiousFloor(self._n_WF, self._p_WF, self._rho,
                             self._k, self._d, self._mu, self._K)
...
def EvaluateFictiousFloorForceDensity(p_WQ, v_WQ, n_WF, p_WF, rho, k, d, mu, K):
    ...
    # Penetration distance time derivative (negative means increasing penetration)
