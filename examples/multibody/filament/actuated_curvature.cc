...
DerModel<double>& RegisterCantileverBeam(
  ...
  /* Create the geometry instance from the shape. */
...
int do_main() {
  ...
  DerModel<double>& der_model = RegisterCantileverBeam(&deformable_model);
  plant.Finalize();
  ...
  const int i_max = FLAGS_simulation_time / FLAGS_time_step;
