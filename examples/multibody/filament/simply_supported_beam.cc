...
DEFINE_double(height, 0.015, "Height of the beam [m].");
DEFINE_int32(num_edges, 100,
             "Number of edges the beam is spatially discretized.");
...
DeformableBodyId RegisterCantileverBeam(
  ...
  Vector3d wall_position = Vector3d(edge_length * 0.001, 0, 0);
  Vector3d wall_normal = Vector3d(1, 0, 0);
  deformable_model->SetWallBoundaryCondition(body_id, wall_position,
                                             wall_normal);
  wall_position = Vector3d(FLAGS_length - edge_length * 0.001, 0, 0);
  wall_normal = Vector3d(-1, 0, 0);
  deformable_model->SetWallBoundaryCondition(body_id, wall_position,
                                             wall_normal);
  return body_id;
}
