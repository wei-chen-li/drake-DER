...
/* A sparse matrix data structure composed of m-by-n block submatrices, where
 each submatrix is of size 3-by-1. Only the non-zero submatrices are stored for
 efficiency. We use `M` to denote `this` Block3x1SparseMatrix throughout this
 class.
 @tparam_default_scalar */
template <class T>
class Block3x1SparseMatrix {
 public:
  DRAKE_DEFAULT_COPY_AND_MOVE_AND_ASSIGN(Block3x1SparseMatrix);

  /* A triplet is a tuple (i, j, value) defining a non-zero submatrix where i is
   the block row index, j is the block column index, and value is a 3x1 dense
   matrix. */
  using Triplet = std::tuple<int, int, Vector3<T>>;

  /* Creates a Block3x1SparseMatrix with capacity for
   `block_rows`-by-`block_cols` block submatrices. The matrix is initialized to
   be empty, i.e. no non-zero blocks. */
  Block3x1SparseMatrix(int block_rows, int block_cols)
  ...
  int cols() const { return block_cols_; }
  ...
  */
  ...
  /* Performs y += Mᵀ * A, where A is also 3x1 block sparse.
   @pre y != nullptr and the sizes of A and y are compatible with this matrix.
  */
  void TransposeAndMultiplyAndAddTo(const Block3x1SparseMatrix<T>& A,
                                    EigenPtr<MatrixX<T>> y) const;

  /* Performs y += Mᵀ * A, where A is 3x3 block sparse.
   @pre y != nullptr and the sizes of A and y are compatible with this matrix.
  */
  ...
  Block3x1SparseMatrix<T> LeftMultiplyByBlockDiagonal(
  ...
  bool operator==(const Block3x1SparseMatrix<T>&) const = default;
