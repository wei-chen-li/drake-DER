...
template <class T>
Block3x3SparseMatrix<T> Block3x3SparseMatrix<T>::LeftMultiplyByBlockDiagonal(
    const std::vector<MatrixX<T>>& Gs, int start, int end) const {
  DRAKE_DEMAND(0 <= start);
  DRAKE_DEMAND(start <= end);
  DRAKE_DEMAND(end < ssize(Gs));
  ...
  Block3x3SparseMatrix<T> A(block_rows(), block_cols());
