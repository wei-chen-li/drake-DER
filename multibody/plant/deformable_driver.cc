  ...
  for (DeformableBodyIndex index(0); index < deformable_model_->num_bodies();
    ...
    for (const FilamentRigidFixedConstraintSpec& spec :
      ...
      if (tree_index.is_valid()) {
        ...
        const SpanningForest::Tree& tree = forest.trees(tree_index);
        MatrixX<T> Jv_v_WBq = MatrixX<T>::Zero(num_constraints, tree.nv());
        Jv_v_WBq.topRows(ssize(spec.nodes) * 3) =
            Jv_v_WBq_block0.middleCols(tree.v_start(), tree.nv());
        ...
        for (int i = 0; i < ssize(spec.edges); ++i) {
          ...
          Jv_v_WBq.template middleRows<1>(offset + i) =
              t_W.transpose() *
              Jv_v_WBq_block.middleCols(tree.v_start(), tree.nv());
        }
