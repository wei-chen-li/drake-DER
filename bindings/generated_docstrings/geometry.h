...
Raises:
    RuntimeError if any measure is not finite positive.)""";
      ...
      // Symbol: drake::geometry::Filament
      struct /* Filament */ {
        // Source: drake/geometry/shape_specification.h
        const char* doc =
R"""(Definition of a general filament.

A filament describes a slender, thread-like shape defined by a
sequence of nodes connected by edges. Each edge is associated with a
material frame, consisting of a unit tangent vector (t) and two
orthonormal material directors (m₁ and m₂), which span the plane of
the cross-section. The filament can either be open-ended or form a
closed loop.)""";
        // Symbol: drake::geometry::Filament::CircularCrossSection
        struct /* CircularCrossSection */ {
          // Source: drake/geometry/shape_specification.h
          const char* doc = R"""()""";
          // Symbol: drake::geometry::Filament::CircularCrossSection::diameter
          struct /* diameter */ {
            // Source: drake/geometry/shape_specification.h
            const char* doc = R"""(Diameter of the circular cross-section.)""";
          } diameter;
        } CircularCrossSection;
        // Symbol: drake::geometry::Filament::Filament
        struct /* ctor */ {
          // Source: drake/geometry/shape_specification.h
          const char* doc_circ =
R"""(Constructs a twist free filament with circular cross-sectioned by
specifying the position of the nodes.

Precondition:
    ``cross_section.radius > 0``.)""";
          // Source: drake/geometry/shape_specification.h
          const char* doc_rect =
R"""(Constructs a twist free filament with rectangular cross-section by
specifying the position of the nodes. Because the cross-section is not
axis-symmetric, the m₁ director of the first frame also need to be
specified.

Precondition:
    ``cross_section.width > 0 && cross_section.height > 0``.

Precondition:
    ``‖first_edge_m1‖ ≈ 1``.

Precondition:
    ``first_edge_m1`` is perpendicular to the first edge vector.)""";
          // Source: drake/geometry/shape_specification.h
          const char* doc_general =
R"""(Constructs a filament with rectangular or circular cross-section by
specifying the position of nodes and the m₁ directors of all frames.

Precondition:
    ``edge_m1.col(i)`` ⊥ ``node_pos.col(i+1) - node_pos.col(i)`` ∀i.

Precondition:
    ``cross_section`` has nonzero size.)""";
        } ctor;
        // Symbol: drake::geometry::Filament::RectangularCrossSection
        struct /* RectangularCrossSection */ {
          // Source: drake/geometry/shape_specification.h
          const char* doc = R"""()""";
          // Symbol: drake::geometry::Filament::RectangularCrossSection::height
          struct /* height */ {
            // Source: drake/geometry/shape_specification.h
            const char* doc =
R"""(Size of the cross-section in the m₂ direction.)""";
          } height;
          // Symbol: drake::geometry::Filament::RectangularCrossSection::width
          struct /* width */ {
            // Source: drake/geometry/shape_specification.h
            const char* doc =
R"""(Size of the cross-section in the m₁ direction.)""";
        ...
        // Symbol: drake::geometry::Filament::closed
        struct /* closed */ {
          // Source: drake/geometry/shape_specification.h
          const char* doc = R"""()""";
        } closed;
        // Symbol: drake::geometry::Filament::cross_section
        struct /* cross_section */ {
          // Source: drake/geometry/shape_specification.h
          const char* doc = R"""()""";
        } cross_section;
        // Symbol: drake::geometry::Filament::edge_m1
        struct /* edge_m1 */ {
          // Source: drake/geometry/shape_specification.h
          const char* doc = R"""()""";
        } edge_m1;
        // Symbol: drake::geometry::Filament::node_pos
        struct /* node_pos */ {
          // Source: drake/geometry/shape_specification.h
          const char* doc = R"""()""";
        } node_pos;
...
R"""(Implementation of QueryObject::ComputeDeformableContact().)""";
        ...
        // Symbol: drake::geometry::GeometryState::ComputeFilamentContact
        struct /* ComputeFilamentContact */ {
          // Source: drake/geometry/geometry_state.h
          const char* doc =
R"""(Implementation of QueryObject::ComputeFilamentContact().)""";
        } ComputeFilamentContact;
...
R"""(Implementation of SceneGraphInspector::GetProximityProperties().)""";
        ...
        // Symbol: drake::geometry::GeometryState::GetReferenceFilament
        struct /* GetReferenceFilament */ {
          // Source: drake/geometry/geometry_state.h
          const char* doc =
R"""(Implementation of SceneGraphInspector::GetReferenceFilament().)""";
        } GetReferenceFilament;
...
Parameter ``frames_per_second``:
    ...
    played back.)""";
        ...
        // Symbol: drake::geometry::MeshcatAnimation::SetObject
        struct /* SetObject */ {
          // Source: drake/geometry/meshcat_animation.h
          const char* doc =
R"""(Set the object at ``frame`` in the animation for a given ``path`` in
the the scene tree.
...
Parameter ``object_json``:
    a json representation of a THREEjs object serialized using
    msgpack.)""";
        } SetObject;
...
Warning:
    ...
    time.)""";
        ...
        // Symbol: drake::geometry::QueryObject::ComputeFilamentContact
        struct /* ComputeFilamentContact */ {
          // Source: drake/geometry/query_object.h
          const char* doc =
R"""(Reports contact information among all filament geometries. It includes
...
Parameter ``filament_contact``:
    Contains all filament contact data on output. Any data passed in
    is cleared before the computation.

Precondition:
    filament_contact != nullptr.
...
Raises:
    ...
    geometry.)""";
        ...
        // Symbol: drake::geometry::SceneGraphInspector::GetReferenceFilament
        struct /* GetReferenceFilament */ {
          // Source: drake/geometry/scene_graph_inspector.h
          const char* doc =
R"""(Returns the reference filament of the geometry with the given
...
Returns:
    A pointer to the reference filament of the geometry if the
    geometry is a filament, or ``nullptr`` otherwise.
