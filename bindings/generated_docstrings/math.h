...
See also:
    DiscardGradient(), drake::ExtractDoubleOrThrow())""";
      ...
      // Symbol: drake::math::FrameTransport
      struct /* FrameTransport */ {
        // Source: drake/math/frame_transport.h
        const char* doc =
R"""(The transport operator maps tᵏ to tᵏ⁺¹ by rotating it φ degrees about
the axis tᵏ × tᵏ⁺¹. Compute d₁ᵏ⁺¹ as the vector rotating d₁ᵏ by φ
degrees about the same axis tᵏ × tᵏ⁺¹.

Parameter ``t_0``:
    The tangent director in frame k.

Parameter ``d1_0``:
    Another director in frame k.

Parameter ``t_1``:
    The tangent director in frame k+1.

Parameter ``d1_1``:
    Mapped director in frame k+1 as described above.
...
R"""(Abbreviation (alias/typedef) for a RollPitchYaw double scalar type.)""";
      ...
      // Symbol: drake::math::RotateAxisAngle
      struct /* RotateAxisAngle */ {
        // Source: drake/math/axis_angle.h
        const char* doc =
R"""(Returns the rotation of ``vec`` around the specified ``axis`` by
``angle``.
...
R"""(Abbreviation (alias/typedef) for a RotationMatrix double scalar type.)""";
      ...
      // Symbol: drake::math::SignedAngleAroundAxis
      struct /* SignedAngleAroundAxis */ {
        // Source: drake/math/axis_angle.h
        const char* doc =
R"""(Returns the signed angle of rotation from ``vec1`` to ``vec2`` around
a given ``axis``. The signed angle is in the range (−π, π]. The angle
is positive if the rotation follows the right-hand rule with ``axis``
as the thumb direction.
...
overloaded function, where A is a matrix of AutoDiffScalar.)""";
      ...
      // Symbol: drake::math::SpaceParallelFrameTransport
      struct /* SpaceParallelFrameTransport */ {
        // Source: drake/math/frame_transport.h
        const char* doc =
R"""(For k = 0,1,..., transform d₁ᵏ to d₁ᵏ⁺¹ using the transport operator
that maps tᵏ to tᵏ⁺¹.

Parameter ``t``:
    The tangent directors in all frames.

Parameter ``d1_0``:
    The director d₁⁰ in frame 0. If not specified, chooses an
    arbitrary director that is perpendicular to t⁰.

Parameter ``d1``:
    Mapped directors in all frames as described above.
...
Precondition:
    ``!d1_0 || d1_0.dot(t.col(0)) ≈ 0``.)""";
      } SpaceParallelFrameTransport;
...
Precondition:
    ...
    of rows.)""";
      ...
      // Symbol: drake::math::TimeParallelFrameTransport
      struct /* TimeParallelFrameTransport */ {
        // Source: drake/math/frame_transport.h
        const char* doc =
R"""(For k = 0,1,..., transform d₁ᵏ(𝑡) to d₁ᵏ(𝑡+𝛥𝑡) using the transport
operator that maps tᵏ(𝑡) to tᵏ(𝑡+𝛥𝑡).

Parameter ``t``:
    The tangent directors at time 𝑡.

Parameter ``d1``:
    The d₁ directors at time 𝑡.

Parameter ``t_next``:
    The tangent directors at time 𝑡+𝛥𝑡.

Parameter ``d1_next``:
    The d₁ directors at time 𝑡+𝛥𝑡.

Precondition:
    ``d1_next != nullptr``.

Precondition:
    Number of columns of ``t``, `d1`, ``t_next``, `d1_next` are the
    same.)""";
      } TimeParallelFrameTransport;
